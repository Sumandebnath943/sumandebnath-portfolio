...
| **Syndication / off-site** | Opened 27 Aug (§1.15), first article shipped 7–8 Sep (§1.23): `what-ai-agents-cost-to-run` on **dev.to, Medium and Hashnode**, all three canonicalised here and all three verified. **Medium self-canonicalises silently — always fetch the tag.** Per-platform rules, the metadata copy-vs-differentiate rule and the JPEG-twin constraint are in **`AEO_PLAYBOOK.md` §6 item 4**; **§6.2 covers cadence** — there is no posting schedule for original work (event-driven, 8–12 a year) and syndication lags publication by 3–7 days. **The 1:1 rule was overturned in one direction on 11 Sep** (§1.26): anything *syndicated* is still a canonical'd copy, but **five Medium exclusives are originals with no copy here and never will have one**. **The canonical is therefore inverted between the two sets** — syndicated copies point home; Medium originals keep Medium's own self-canonical, because "originally published elsewhere" would point at a 404. Getting this backwards is the easiest mistake in the whole process. Read both before syndicating the next one. |
