...
const SOCIAL_PATHS: Record<string, string> = {
  ...
  // dev.to. Taken from the pinned `simple-icons` package (`siDevdotto`) rather
  // than transcribed, because a path copied by eye renders as a smear and the
  // failure is silent — it still draws something.
  devto:
...
const socials = [
  ...
  // Added 7 Sep 2026, at the same time as `sameAs` and the rel="me" link in
  // app/layout.tsx. All three are the same assertion in three languages and a
  // profile in one but not the others is a claim half-made — the state
  // HuggingFace sat in for months.
  { id: "devto", label: "DEV", href: "https://dev.to/suman_debnath_1" },
  // Added 8 Sep 2026, completing the syndication set. Nine pills is the most
  // this row should carry — it wraps to two lines on a phone already, and the
  // point of the row is that a reader can take it in, not that it lists
  // everything. A tenth profile needs an argument, not just an account.
  { id: "medium", label: "Medium", href: "https://medium.com/@sumandebnath943" },
  { id: "hashnode", label: "Hashnode", href: "https://hashnode.com/@sumandebnath" },
];
